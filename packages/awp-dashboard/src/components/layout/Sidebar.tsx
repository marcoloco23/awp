...
const federationItems = [
  { href: "/sync", label: "Federation", icon: RefreshCw },
];
  ...
  return (
    ...
    >
      ...
      <nav aria-label="Main navigation" className="flex-1 py-3 px-3 space-y-0.5 overflow-y-auto">
        ...
        <div className="text-[10px] font-mono text-[var(--text-muted)] tracking-wider uppercase px-2.5 py-2 mt-3">
          Federation
        </div>
        {federationItems.map(({ href, label, icon: Icon }, i) => {
        ...
        {societyItems.map(({ href, label, icon: Icon }, i) => {
          const isActive = pathname.startsWith(href);
          return (
            <motion.div
              key={href}
